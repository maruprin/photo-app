...
//console.log(dispatch(fetchPhotos()))
    ...
    return (
        <>
            <input className='input-search' onChange={(e)=>console.log(e.target.value)}type='text' placeholder='search your photos' />
            <input type='submit' value='click' onClick={handleClick} />
            <ImageList sx={{ width: '90%', margin: '0 auto'}}>
              {photosRes.resultsStatic.map((item,i) => (
                <ImageListItem key={i}>
                  <img
                    src={`${item.links.download}?w=248&fit=crop&auto=format`}
